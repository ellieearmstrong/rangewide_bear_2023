...
# Create comma-separated chromosome list for bcftools
chrom_list=$(IFS=,; echo "${CHROMS[*]}")

echo "Starting VCF splitting for SFS analysis..."
echo "Input VCF: allbears.M2.repmap1.indels.DPfilt.QUAL30.drop20miss4dp.AN522.snps.nonref.rename.singletoncorr.norel.nocap.vcf.gz"
echo "Output directory: filtered_vcf_sfs"
echo "Number of chromosomes: ${#CHROMS[@]}"

# Process each population
for pop in "${POPS[@]}"; do
    echo "Processing population: ${pop}"
    
    # Create temporary directory for this population
    temp_dir="temp_${pop}"
    mkdir -p ${temp_dir}
    
    # Create individuals list for this population
    echo "${POP_SAMPLES[$pop]}" | tr ',' '\n' > ${temp_dir}/${pop}_individuals.list
    
    # Count individuals in this population
    n_individuals=$(wc -l < ${temp_dir}/${pop}_individuals.list)
    echo "  - Found ${n_individuals} individuals"
    
    # Extract samples for this population and filter sites with no missing data
    # Step 1: Extract samples and chromosomes
    # Step 2: Filter for no missing data (F_MISSING calculated on subset)
    bcftools view \
        -r ${chrom_list} \
        -S ${temp_dir}/${pop}_individuals.list \
        allbears.M2.repmap1.indels.DPfilt.QUAL30.drop20miss4dp.AN522.snps.nonref.rename.singletoncorr.norel.nocap.vcf.gz | \
    bcftools view \
        -i 'F_MISSING==0' \
        -O z \
        -o filtered_vcf_sfs/${pop}.vcf.gz
    
    echo "  - Calculating depth per individual..."
    vcftools --gzvcf filtered_vcf_sfs/${pop}.vcf.gz \
             --depth \
             --out filtered_vcf_sfs/${pop}
    
    if [[ -f "filtered_vcf_sfs/${pop}.idepth" ]]; then
        echo "  - Depth output: filtered_vcf_sfs/${pop}.idepth"
        
        echo "  - Depth summary:"
        tail -n +2 "filtered_vcf_sfs/${pop}.idepth" | \
        awk '{sum+=$3; if(NR==1){min=max=$3} else {if($3<min) min=$3; if($3>max) max=$3}} 
             END {printf "    Mean: %.2f, Min: %.2f, Max: %.2f\n", sum/NR, min, max}'
    fi
    
    echo "  - Completed processing ${pop}"
    
    # Clean up temporary files for this population
    rm -rf ${temp_dir}
done

echo "All populations completed at $(date)"